...
// ─── Liste des instances user-service ────────────────────────────────────────

const TARGETS = (
  process.env.USER_TARGETS ||
  "http://user-service-1:3002,http://user-service-2:3002"
)
  .split(",")
  .map((u) => u.trim());
...
app.all("/{*splat}", async (req, res) => {
  ...
  try {
    ...
    res.status(502).json({ error: "User service indisponible" });
  }
});
